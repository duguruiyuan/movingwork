...
run;
PROC SORT DATA=sino_loan_n OUT=LOAN_N1 ;
...
/*申请信息*/
data sino_loan_apply_n;
 set SSS.SINO_LOAN_APPLY(WHERE=(SUBSTR(SORGCODE,1,1)='Q' AND 
                                SORGCODE not in ('Q10152900H0000' 'Q10152900H0001') AND 
                                DATEPART(DGETDATE) <= &STAT_DT.));
run;
PROC SORT DATA=sino_loan_apply_n OUT= APPLY_N1;
...
/*特殊交易信息*/
data sino_loan_spec_trade_n;
 set SSS.Sino_LOAN_SPEC_TRADE(WHERE=(SUBSTR(SORGCODE,1,1)='Q' AND 
                                     SORGCODE not in ('Q10152900H0000' 'Q10152900H0001') AND 
                                     DATEPART(DGETDATE) <= &STAT_DT.));
run;
PROC SORT DATA=sino_loan_spec_trade_n OUT=SPEC_N1;
...
run;
PROC SORT DATA=sino_loan_o OUT=LOAN_O1 ;
	BY SORGCODE SACCOUNT DGETDATE;
RUN;
...
/*申请信息*/
data sino_loan_apply_o;
 set SSS.SINO_LOAN_APPLY(WHERE=(SUBSTR(SORGCODE,1,1)='Q'  AND 
                                SORGCODE not in ('Q10152900H0000' 'Q10152900H0001') AND 
                                DATEPART(DGETDATE) <= &LAST_DT.));
run;
PROC SORT DATA=sino_loan_apply_o OUT= APPLY_O1;
	BY SORGCODE SAPPLYCODE SCERTNO;
RUN;
...
/*特殊交易信息*/
data sino_loan_spec_trade_o;
 set SSS.Sino_LOAN_SPEC_TRADE(WHERE=(SUBSTR(SORGCODE,1,1)='Q' AND 
                                     SORGCODE not in ('Q10152900H0000' 'Q10152900H0001') AND 
                                     DATEPART(DGETDATE) <= &LAST_DT.));
run;
PROC SORT DATA=sino_loan_spec_trade_o OUT=SPEC_O1;
	BY SORGCODE SACCOUNT;
RUN;
DATA SPEC_O2;
	SET SPEC_O1;
