...
/*注意修改时间段;*/
%LET END=MDY(5,31,2016);
%LET START=MDY(5,1,2016);
...
/*		,PUT(sorgcode,$SHORT_cd.) as shortname LABEL="机构简称"*/
	...
	SELECT 
		...
		,SUM(CASE WHEN DATEPART(DGETDATE) <= &END. THEN 1 ELSE 0 END) AS NOW
		,SUM(CASE WHEN DATEPART(DGETDATE) < &START_THREE. THEN 1 ELSE 0 END) AS LAST_THREE
	FROM NFCS.SINO_LOAN(keep = SORGCODE iloanid DGETDATE istate WHERE=(SUBSTR(SORGCODE,1,1)='Q' AND SORGCODE^='Q10152900H0000' AND SORGCODE^='Q10152900H0001' and istate ne -1)) AS P
...
PROC SORT DATA=NFCS.Sino_LOAN_SPEC_TRADE(keep = SORGCODE SACCOUNT dgetdate istate WHERE=(SUBSTR(SORGCODE,1,1)='Q' AND SORGCODE not in ('Q10152900H0000' 'Q10152900H0001') and istate ne -1)) OUT=SPEC_N1;
